...
class InviteDetails extends StatefulWidget {
  final type;
  final id;
  final name;
  final email;
  ...
  @override
  _InviteDetailsState createState() => _InviteDetailsState();
}

class _InviteDetailsState extends State<InviteDetails> {
  ...
  Widget build(BuildContext context) {
    ...
    return Theme(
      ...
      child: StreamBuilder(
          stream: widget.type == 'invite' ? FirebaseFirestore.instance.collection('Users')
              .doc(widget.uid)
              .collection('Invited').doc(widget.id).snapshots() : FirebaseFirestore.instance.collection('Users')
              .doc(widget.uid)
              .collection('Cohost').doc(widget.id).snapshots(),
          builder: (context, snapshot1) {
            ...
            var meetDoc1 = snapshot1.data;
            var id = meetDoc1['id'];
            var topic = meetDoc1['meetName'];
            var host = meetDoc1['name'];
                                      ...
                                      await FirebaseFirestore.instance.doc("Meetings/$id").get().then((doc) async {
                                        if (doc.exists) {
                                          if (meetDoc['hasStarted'] == true) {
                                            if (widget.type == 'cohost') {
                                              _coHostMeeting(
                                                  id, topic, meetDoc['chat'],);
                                            } else {
                                            _joinMeeting(
                                              id, topic,
                                              meetDoc['chat'],
                                            ...
                                            _notStarted(id, topic, widget.type, meetDoc['chat'],
                                                meetDoc['live'],
                                                meetDoc['record'],
                                                meetDoc['raise'],
                                                meetDoc['yt'],
                            ...
                            child: RaisedButton(
                                ...
                                onPressed: () async {
                                    final action = await Dialogs.redAlertDialog(
                                        context,
                                        'Delete Invitation',
                                        'Do You Really want to delete this meeting invitation?',
                                        'Delete',
                                        'Cancel');
  ...
  void _notStarted(id, topic, type, ch, live, record, raise, yt, kick) {
    showDialog(
      context: context,
  ...
  _joinMeeting(id, topic, bool ch, bool live, bool raise, bool record, bool yt,
      bool kick) async {
    String serverUrl =
      ...
      if (connectivityResult == ConnectivityResult.none) {
        ...
        await JitsiMeet.joinMeeting(
          ...
          listener: JitsiMeetingListener(onConferenceWillJoin: (message) {
            ...
            databaseService.addMyJoined(options.room, widget.uid, options.subject, DateTime.now().subtract(Duration(seconds: 2)), widget.name, widget.email, widget.PhotoUrl, 'invited');
            setState(() {
              inMeeting = true;
            });
            Fluttertoast.showToast(
  ...
  _coHostMeeting(id, topic, ch) async {
    String serverUrl =
    serverText.text?.trim()?.isEmpty ?? "" ? null : serverText.text;
      ...
      if (connectivityResult == ConnectivityResult.none) {
        ...
        await JitsiMeet.joinMeeting(
          ...
          listener: JitsiMeetingListener(onConferenceWillJoin: (message) {
            ...
            databaseService.addMyJoined(options.room, widget.uid, options.subject, DateTime.now().subtract(Duration(seconds: 2)), widget.name, widget.email, widget.PhotoUrl, 'cohost');
            Fluttertoast.showToast(
                msg: 'Meeting Started',
                toastLength: Toast.LENGTH_SHORT,
                gravity: ToastGravity.SNACKBAR,
  ...
  _shareMeeting(
    ...
    if (connectivityResult == ConnectivityResult.none) {
      ...
      final textshare =
          "${widget.name} (Co-host) is inviting you to join a Just Meet meeting.  "
          "\n"
          "\n"
          "Meeting Id - $roomids.  "
          "\n"
