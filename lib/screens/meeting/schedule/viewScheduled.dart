...
class _viewScheduledState extends State<viewScheduled> {
  ...
  Widget build(BuildContext context) {
    ...
    var from = meetDoc['from'];
    var to = meetDoc['to'];
    var date = meetDoc['date'];
    var time = DateFormat.jm()
        .format(DateFormat("hh:mm").parse(from));
    var timeTo = DateFormat.jm()
        .format(DateFormat("hh:mm").parse(to));
    DateTime tomorrow = now.add(Duration(days: 1));
    DateTime daytomorrow = now.add(Duration(days: 2));
    String formattedDate =
    dateFormat.format(DateTime.parse(date));
    var finalDate;
    if (DateTime.parse(date).day == now.day &&
    DateTime.parse(date).month == now.month &&
    DateTime.parse(date).year == now.year) {
    finalDate = 'Today';
    } else if ((DateTime.parse(date).day == tomorrow.day &&
    DateTime.parse(date).month == tomorrow.month &&
    DateTime.parse(date).year == tomorrow.year)) {
    finalDate = 'Tomorrow';
    } else if ((DateTime.parse(date).day ==
    daytomorrow.day &&
    DateTime.parse(date).month == daytomorrow.month &&
    DateTime.parse(date).year == daytomorrow.year)) {
    ...
    return Scaffold(
                        ...
                        children: <Widget>[
                                ...
                                onPressed: () async {
                                  ...
                                  if (connectivityResult ==
                                    ...
                                    if (action == DialogAction.yes) {
                                      _hostScheduledMeeting(id, topic, meetDoc['chat'], meetDoc['live'], meetDoc['record'], meetDoc['raise'], meetDoc['yt'], meetDoc['kick'], meetDoc['repeat'], meetDoc['date'], meetDoc['from'], meetDoc['to']);
                                    }
                          ...
                          SizedBox(
                            ...
                            child: OutlineButton(
                                ...
                                onPressed: () async {
                                  _navigateToNote(id, topic, meetDoc['date'], meetDoc['from'], meetDoc['to'], meetDoc['repeat'], meetDoc['chat'], meetDoc['live'], meetDoc['record'], meetDoc['raise'], meetDoc['yt'], meetDoc['kick']);
                                },
                                child: Text(
                                  'Edit',
                                  style: TextStyle(
      ...
      'Cancel');
      if (action == DialogAction.yes) {
        databaseService.removeMyScheduled(id, widget.uid);
  ...
  _hostScheduledMeeting(id, topic, ch, live, record, raise, yt, kick, repeat, date, from, to) async {
    String serverUrl =
  ...
  void _navigateToNote(id, meetName, date, from, to, repeat, ch, live, record, raise, yt, kick) {
   Navigator.push(
      context,
      CupertinoPageRoute(builder: (context) => NoteScreen(widget.uid, widget.email, id, meetName, date, from, to, repeat, ch, live, record, raise, yt, kick)),
    );
  }
}
